...
class GroqProvider(OpenAILikeProvider):
    """Groq OpenAI-compatible provider implementation."""
    ...
    def check(self, token: str, address: str = "", endpoint: str = "", model: str = "") -> CheckResult:
        """Check Groq token validity with the lightweight models endpoint."""
        ...
        if code == 0:
            logger.debug(f"Check Groq models failed: {message}")
    ...
    def _judge_models(self, code: int, message: str) -> CheckResult:
        """Judge Groq models.list response."""
        if code == 200:
            try:
