...
"""AgnesAI push service — reads agnes-ai valid-keys.txt after a scan completes and
pushes the validated generic ``sk-`` keys to a gpt-load instance via POST
...
Deliberately mirrors ``web.serpapi_push.py`` but stays a separate module: the
target instance is configured via its own env vars (``AGNES_LOAD_BASE_URL`` /
``AGNES_LOAD_GROUP_ID`` / ``AGNES_LOAD_AUTH_KEY``), keys are pre-filtered to
generic ``sk-`` keys (anthropic ``sk-ant-``, project ``sk-proj-`` and service
account ``sk-svcacct-`` prefixes excluded), batches are chunked at 500 keys per
POST, and the provider gate is ``agnes-ai``. Env-driven with a default instance,
so production keeps working until the target is reconfigured.
"""
...
logger = get_logger("web.agnes_ai_push")
...
class AgnesAIPushService:
    """Service that pushes validated agnes-ai sk- keys to a gpt-load instance.

    Lifecycle:
        1. ``push_valid_keys(provider, run_id)`` — called from scan thread
        2. Skips unless provider == "agnes-ai" and a base url is configured
        3. Reads valid-keys.txt from the workspace providers dir
        4. Pre-filters to generic sk- keys; other lines count as ignored
    ...
    def __init__(
        ...
        if base_url is None:
            base_url = os.environ.get("AGNES_LOAD_BASE_URL", _DEFAULT_BASE_URL)
        if auth_key is None:
            auth_key = os.environ.get("AGNES_LOAD_AUTH_KEY", "")
        if group_id is None:
            try:
                group_id = int(os.environ.get("AGNES_LOAD_GROUP_ID", str(_DEFAULT_GROUP_ID)))
            except ValueError:
    ...
    def push_valid_keys(self, provider_name: str, run_id: str) -> None:
        """Read valid-keys.txt and push valid sk- keys to the gpt-load instance.
        ...
        try:
            logger.info(
                f"AgnesAI push started: provider={provider_name} run_id={run_id}"
            )

            # 1. Gate: only for agnes-ai runs with a configured base url
            if provider_name != _PROVIDER_NAME or not self._base_url:
                logger.info(
                    f"AgnesAI push skipped: provider={provider_name} "
                    f"(base url not configured or not an agnes-ai run)"
                )
                return
            ...
            with self._lock:
                if run_id in self._seen_run_ids:
                    logger.info(
                        f"AgnesAI push skipped: run_id={run_id} already pushed"
                    )
                    return
            ...
            # 4. Pre-filter: only generic sk- keys are pushed
            valid = [key for key in lines if _KEY_RE.fullmatch(key)]
            if not valid:
                logger.info(
                    f"No sk- keys found for provider '{provider_name}' "
                    f"— skipping push"
                )
                return
            ...
            logger.info(
                f"AgnesAI push complete: provider={provider_name} run_id={run_id} "
                f"status={status} added={added_count} ignored={ignored_count}"
            )

        except Exception as exc:
            logger.error(
                f"AgnesAI push failed unexpectedly: provider={provider_name} "
    ...
    def _push_chunk(self, chunk: list[str]) -> tuple[str, int, int, str | None]:
        ...
        outcome is ``"success"`` (HTTP 200) or ``"failed"`` (non-retryable 4xx,
        retry exhaustion, unexpected errors). Retries on 429/5xx and network
        errors with exponential backoff (1s/3s/9s).
        """
        ...
        for attempt in range(1 + _MAX_RETRIES):  # initial + retries
            ...
            if attempt < _MAX_RETRIES:
                ...
                logger.warning(
                    f"AgnesAI push attempt {attempt + 1} failed ({last_error}), "
...
_agnes_ai_push_service: AgnesAIPushService | None = None


def get_agnes_ai_push_service() -> AgnesAIPushService:
    """Return the module-level AgnesAIPushService singleton."""
    global _agnes_ai_push_service
    if _agnes_ai_push_service is None:
        _agnes_ai_push_service = AgnesAIPushService()
    return _agnes_ai_push_service