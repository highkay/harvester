...
| Grok / xAI | `grok` | `xai-...` API keys via `XAI_API_KEY` env assignments + `api.x.ai` domain dorks | `GET /v1/models` against `https://api.x.ai/v1` | Web/SSO session tokens are NOT scanned by the preset; session traces matched by the provider route to `wait-check-keys.txt` (browser-context manual verification), never `valid-keys.txt` |
...
| Agnes AI | `agnes-ai` | `sk-...` API keys (env-name anchored; `sk-ant-`/`sk-proj-`/`sk-svcacct-` excluded) | minimal chat-completion probe (`agnes-2.5-flash`, `max_tokens=1`) | Base `https://apihub.agnes-ai.com/v1`; `GET /v1/models` is presence-only and NOT used for validation |
| DeepSeek | `deepseek` | DeepSeek env-anchored `sk-...` keys (`DEEPSEEK_*` assignments; measured: `sk-` + 32 hex, n=401; `api.deepseek.com` dorks widen to Bearer/quoted/plain) | `GET /models` gate + minimal chat-completion probe (`max_tokens=1`) to surface 402 | Default base URL: `https://api.deepseek.com`; 401 body may be non-JSON; zero-balance keys map to `no-quota-keys.txt` |
| Kimi / Moonshot | `kimi` | Moonshot env-anchored `sk-...` keys (measured: `sk-` + 48 alnum; domain-named dorks widen to Bearer/quoted) | `GET /v1/models` | Default base URL: `https://api.moonshot.cn/v1`; quotas map to `no-quota-keys.txt` |
| GLM / Zhipu | `glm` | `{id}.{secret}` dot-form API keys | Chat completion probe (`glm-5.3-flash`) | Default base URL: `https://open.bigmodel.cn/api/paas/v4`; no `/models` endpoint, so inspect is skipped |
| Xiaomi MiMo | `mimo` | Token-Plan env-anchored `tp-...` keys (`MIMO_*`/`XIAOMIMIMO_*` assignments; measured: `tp-` + 48 alnum, n=1; `xiaomimimo.com` host dorks widen) | `GET /models` | Default base URL: `https://token-plan-cn.xiaomimimo.com/v1` (CN); regional clusters per task (`token-plan-sgp.xiaomimimo.com/v1` for Singapore); `sk-` pay-as-you-go keys (api.xiaomimimo.com) are NOT scanned — unmeasured vendor surface, re-flood guard |
| Alibaba Qwen (DashScope) | `qwen` | DashScope env-anchored `sk-...` keys (`DASHSCOPE_*`/`QWEN_*` assignments; measured: `sk-` + 32 hex, n=315; `dashscope.aliyuncs.com` / `dashscope-intl.aliyuncs.com` host dorks widen) | `GET /models` + chat probe | Default base URL: `https://dashscope.aliyuncs.com/compatible-mode/v1` (CN); `dashscope-intl.aliyuncs.com` for international; zero-balance is HTTP 400 + `code: Arrearage` (mapped to no-quota) |
| ModelScope (魔搭) | `modelscope` | `ms-`-prefixed tokens via env-anchored assignments (`MODELSCOPE_API_KEY` / `MODELSCOPE_SDK_TOKEN` / `MODELSCOPE_ACCESS_TOKEN` / `MODELSCOPE_API_TOKEN`) + domain dorks (`modelscope.cn` / `api-inference.modelscope.cn` Bearer + `oauth2:` git URLs) | `GET https://modelscope.cn/openapi/v1/users/me` (Bearer) hub account probe; single-call validity | Single CN endpoint; `api-inference.modelscope.cn/v1/models` is presence-only (200 for any key) and NOT used for validation |
...
- The `serpapi` preset scans `SERPAPI_API_KEY` / `SERPAPI_KEY` / `SERP_API_KEY` assignments through GitHub API search (context-anchored extraction, prefix-less 64-char hex keys (measured on prod: valid keys are 64 hex chars)) and validates/audits them with SerpApi's free `account.json` Account API.
   - The `grok` preset scans `xai-` API keys (`XAI_API_KEY` assignments + `api.x.ai` dorks) and validates them against xAI's `GET /v1/models`. Web/SSO session tokens are intentionally NOT scanned by the preset; session traces matched by the provider route to `wait-check-keys.txt` instead.
